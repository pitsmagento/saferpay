...
class PaymentAdapter extends BuildContainer implements PaymentAdapterInterface
{
    ...
    public function buildInitializeBodyData($bodyData)
    {
        ...
        if($bodyData['AuthorisationMethod'] != Constants::HOSTED_FIELD) {
            ...
            } elseif(isset($bodyData['card_holdername_display']) &&
                $bodyData['card_holdername_display'] == Constants::ACTIVE) {
                $initializeData['CardForm'] = [
                    'HolderName' => Constants::CARD_HOLDER_NAME_MANDATORY
                ];
            }
        }
        if ($bodyData['AuthorisationMethod'] == Constants::PAYMENT_PAGE) {
...
//            }
        ...
        if (isset($bodyData['force_sca_check']) && $bodyData['force_sca_check'] == Constants::ACTIVE) {
            $initializeData['Authentication'] = [
